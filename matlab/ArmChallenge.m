...
while true
    ...
    elseif state == "SM_SEND_NEXT_POSITION":
        ...
        disp("ActPln.->Sending position " + string(idx) + " = " + string(predefined_positions(idx)))
        goal_q = predefined_positions(idx);
        send(calculate_trajectory(goal_q, sub_current_q), pub_goal_pose);
    ...
    elseif state == "SM_RECOGNIZE_OBJECTS":
        ...
        if nearest_obj != [0,0,0]
            disp("Highest object at: " + string(target_object));
    ...
    elseif state == "SM_INVERSE_KINEMATICS":
        ...
        goal_q = inverse_kinematics(target_obj(1), target_obj(2), target_obj(3) + 0.15, robot, sub_current_q);
    ...
    elseif state == "SM_LIFT_OBJECT"
        disp("Trying to calculate inverse kinematics for pose " + string(target_object))
        goal_q = inverse_kinematics(target_obj(1), target_obj(2), target_obj(3) + 0.25, robot, sub_current_q);
        disp("Sending object position: " + string(goal_q))
        send(calculate_trajectory(goal_q, sub_current_q), pub_goal_pose);
